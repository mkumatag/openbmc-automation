...
Turn ON the Power LED
   [Documentation]   This testcase is to test the setOn functionality of the
   ...               Power LED. The LED state is read again to check if
   ...               the LED is in ON state.
   Set On   power
   ${ledstate} =   Get LED State   power
   should be equal as strings   ${ledstate}   On
...
Blink Fast the Power LED
   [Documentation]   This testcase is to test the setBlinkFast functionality of the
   ...               Power LED. The LED state is sampled to figure out
   ...               whether the LED is blinking. There is no distinguishing
   ...               between whether the LED is blinking fast or slow for
   ...               this testcase to pass.
   ${OFF_VALUE}=   Set Variable   ${0}
...
Get LED State
   [arguments]    ${args}
   ${data} =   create dictionary   data=@{EMPTY}
   ${resp} =   OpenBMC Post Request   /org/openbmc/controller/led/${args}/action/GetLedState   data=${data}
...
Set On
   [arguments]    ${args}
   ${data} =   create dictionary   data=@{EMPTY}
   ${resp} =   OpenBMC Post Request   /org/openbmc/controller/led/${args}/action/setOn   data=${data}
   should be equal as strings   ${resp.status_code}   ${HTTP_OK}
   ${json} =   to json   ${resp.content}
...
Set Off
   [arguments]    ${args}
   ${data} =   create dictionary   data=@{EMPTY}
   ${resp} =   OpenBMC Post Request   /org/openbmc/controller/led/${args}/action/setOff   data=${data}
   should be equal as strings   ${resp.status_code}   ${HTTP_OK}
   ${json} =   to json   ${resp.content}
   should be equal as integers   ${json['data']}   0
...
Set Blink Fast
   [arguments]    ${args}
   ${data} =   create dictionary   data=@{EMPTY}
   ${resp} =   OpenBMC Post Request   /org/openbmc/controller/led/${args}/action/setBlinkFast   data=${data}
   should be equal as strings   ${resp.status_code}   ${HTTP_OK}
   ${json} =   to json   ${resp.content}
   should be equal as integers   ${json['data']}   0
...
Set Blink Slow
   [arguments]    ${args}
   ${data} =   create dictionary   data=@{EMPTY}
   ${resp} =   OpenBMC Post Request   /org/openbmc/controller/led/${args}/action/setBlinkSlow   data=${data}
   should be equal as strings   ${resp.status_code}   ${HTTP_OK}
   ${json} =   to json   ${resp.content}
   should be equal as integers   ${json['data']}   0
